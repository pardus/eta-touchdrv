...
static ssize_t otd_write(struct file * filp,
...
static long set_report(device_context * otd, unsigned short length, void
  const * data) {
  void * kernel_data;
  int r;
...
static long get_report(device_context * otd, unsigned short length, void * data) {
  ...
  do {
    ...
    r = usb_control_msg(otd -> usb_device, usb_rcvctrlpipe(otd -> usb_device, 0), 0, 0xc0, 0x300 | (((unsigned char
      const * ) kernel_data)[0] & 0xff), 0, kernel_data, length, 1000);
    if (r >= 0) {
...
static long sync_multitouch(device_context * otd, unsigned short length, void
  const * data) {
...
static long otd_unlocked_ioctl(struct file * filp, unsigned int ctl_code, unsigned long ctl_param) {
  ...
  case OTD_IOCTL_CODE_TYPE_SET_REPORT:
    return set_report(otd, ctl_code & OTD_IOCTL_CODE_LENGTH_MASK, (void
      const * ) ctl_param);
  ...
  case OTD_IOCTL_CODE_TYPE_SYNC_MULTITOUCH:
    return sync_multitouch(otd, ctl_code & OTD_IOCTL_CODE_LENGTH_MASK, (void
      const * ) ctl_param);
  }
  return 0;
}
...
static int otd_probe(struct usb_interface * intf,
...
static void otd_disconnect(struct usb_interface * intf) {
  ...
  //cdev����Դ��cdev_alloc��ɾ�����ܵ����ڴ�й©
  //cdev_del(&otd->cdev);
  device_destroy(otd_class, otd -> dev);
  unregister_chrdev_region(otd -> dev, 1);
  usb_set_intfdata(intf, NULL);
  input_unregister_device(otd -> input_dev);
  //ԭ��û�е���input_free_device
  input_free_device(otd -> input_dev);
  kfree(otd);
}
